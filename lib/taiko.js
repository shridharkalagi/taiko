...
const getElementGetter = (selector, query, tags) => {
    ...
    if (selector.attrValuePairs) {
        let query = tags.split('|').map(tag => getQuery(selector.attrValuePairs, tag)).join('|');
        get = async () => await handleRelativeSearch(await $$xpath(query), selector.args);
    }
    else if (selector.label) get = async () => await handleRelativeSearch(await query(), selector.args);
    else get = async () => await handleRelativeSearch(await $$xpath(tags), selector.args);
    return get;
};
